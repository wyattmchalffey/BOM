...
if not ok_ssl then ssl = nil end
...
local connect = function(self,ws_url,ws_protocol,ssl_params)
  ...
  if protocol == 'wss' then
    if not ssl then
      return nil, 'wss requires LuaSec (ssl module not found)', nil
    end
  ...
  if type(ws_protocol) == 'string' and ws_protocol ~= '' then
      ws_protocols_tbl = {ws_protocol}
  elseif type(ws_protocol) == 'table' then
      ws_protocols_tbl = ws_protocol
  end
  local key = tools.generate_key()
  -- Do not include default ports in Host header (80 for ws, 443 for wss)
  local req_port = port
  if (protocol == 'wss' and port == 443) or (protocol == 'ws' and port == 80) then
    req_port = nil
  end
