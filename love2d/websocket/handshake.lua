...
local upgrade_request = function(req)
  ...
  -- Only include Sec-WebSocket-Protocol if protocols are provided and non-empty
  if req.protocols then
    local joined = table.concat(req.protocols, ', ')
    if joined ~= '' then
      tinsert(lines, format('Sec-WebSocket-Protocol: %s', joined))
    end
  end
  if req.origin then
